  ...
  userSearchInput.addEventListener('input', async (e) => {
    ...
    if (users.length > 0) {
      users.forEach(u => {
        ...
        avatar.textContent = u.full_name.charAt(0);
        ...
        handle.className = 'contact-last-msg';
...
function startPersonalChat(user) {
  ...
  const avatar = document.createElement('div');
  avatar.className = 'contact-avatar';
  avatar.textContent = user.full_name.charAt(0);
  ...
  lastMsg.textContent = `Start chatting with @${user.username}`;
  ...
  contactItem.onclick = () => {
    ...
    if (chatMessages) {
      ...
      welcomeMsg.textContent = `Hi! This is the start of your personal chat with ${user.full_name}. `;

      const timeSpan = document.createElement('span');
      timeSpan.className = 'message-time';
      timeSpan.textContent = 'Just now';

      welcomeMsg.appendChild(timeSpan);
